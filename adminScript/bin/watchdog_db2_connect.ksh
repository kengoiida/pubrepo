...
do
    cat ${TARGET_LIST} |egrep "^${HOSTNAME}:|^COMMON:"|awk -F: '{ if($2 == "'$WATCH_KIND'" && $3 == "'$inst'" ) {print $3}}' |sed 's/\%//g' | read wdtarget
    ...
    do
        ...
        if [[ $RC != 0 ]]; then
            getMessage "WD0011C";logWriter ${LOGFILE} "${dbname} ${message}"; continue
        fi
